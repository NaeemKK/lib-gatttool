...
static void conn_cb(GAttrib *attrib, GIOChannel *io, GError *err, gpointer user_data)
{
	uint16_t mtu;
	uint16_t cid;
		...
		const char *dst_type, int mtu, int psm, const char *sec_level,dev_connect_cb cb,void *user_data)
{
	...
	*conn_handle->dst_addr = g_strdup(dst);
	...
	data_connect_cb_t *u_data = gmalloc(sizeof(data_connect_cb_t));
	if(!u_data)
	{
		err_code = MEM_ISSUE;
		goto err;
	...
	*conn_handle->iochannel = mod_gatt_connect(opt_interface, dst, opt_dst_type, opt_sec_level,
					psm, mtu, cb,conn_cb,u_data, &gerr);
